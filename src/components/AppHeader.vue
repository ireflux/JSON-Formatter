<template>
  <header>
    ...
    <div class="header-buttons">
      <button @click="$emit('copy')" class="copy-btn" :disabled="!canCopy || isBusy">
        <span class="button-content">{{ isCopying ? 'Copying...' : 'Copy' }}</span>
      </button>
      <button @click="$emit('compress')" class="compress-copy-btn" :disabled="!canCopy || isBusy">
        <span class="button-content">{{ isCompressing ? 'Compressing...' : 'Compress & Copy' }}</span>
      </button>
      <button @click="$emit('toggle-diff')" class="diff-btn" :class="{ active: isDiffMode }" :disabled="isBusy">
...
defineEmits(['copy', 'compress', 'toggle-diff'])
</script>
